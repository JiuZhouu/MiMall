        ...
        <div class="cart-box">
          ...
          <ul class="cart-item-list">
            <li class="cart-item" v-for="(item,index) in list" v-bind:key="index">
              <div class="item-check">       <!--当属性true的时候才有这个class-->
  ...
  export default{
    ...
    methods:{
      ...
      renderData(res){
        ...
        this.checkedNum = this.list.filter(item=>item.productSelected).length;  //filter过滤 选中的进行返回
      },
      // 购物车下单
      order(){
        let isCheck = this.list.every(item=>!item.productSelected); //每一项都不是选中状态
        if(isCheck){
