    ...
    <div class="wrapper">
      <div class="container">
        <div class="login-form">
          ...
          <div class="btn-box">
            <a href="javascript:;" class="btn" @click="login">登录</a>     <!--@也可以写成 v-on: 调用了login-->
          </div>
          <div class="tips">
...
export default {
  ...
  data(){
    return {
      username:'',
      password:'', 
      userId:'',        //保存用户id 传凭证给后端 每次发请求的时候把id传给后端 根据用户id进行操作
    }
  },
  methods:{                //定义方法
    login(){
      let { username,password } = this;             //获取参数
      this.axios.post('/user/login',{                 //地址+传参
        username,
        password
      }).then((res)=>{
        this.$cookie.set('userId',res.id,{expires:'Session'}); //expires是过期时间
        //this.$store.dispatch('saveUserName',res.username);
        this.saveUserName(res.username);
        this.$router.push({
          /*path:'/index',
          query:{
            from:'login'  //从login过来的
          }*/ //query显式方式
          name:'index',
          params:{
            from:'login'
